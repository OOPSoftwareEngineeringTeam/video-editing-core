...
import AudioInsertForm from "../AudioInsertForm/AudioInsertForm";
...
const VideoInput = () => {
  ...
  const [audioGallery, setAudioGallery] = useState();
  ...
  const [opendialogInsertAudio, setOpendialogInsertAudio] = useState(false);
  ...
  useEffect(() => {
    ...
    const getGallery = async () => {
      try {
        ...
        var responseA = await videoEditingApi.getGallery(2);
        ...
        if (responseL.data.length > 0) {
          ...
          if (responseA.data.length > 0) {
            responseA.data.forEach((element, index) => {
              const data = {
                label: element.event,
                file_name: element.file_name,
                width: element.width,
  ...
  const handleAddMusic = (value) => {
    setOpendialogInsertAudio(false);
    console.log(value)
    if (value)
      setAudio({ file_name: value.file_name, startTime: value.width, endTime: value.height })
  };
  ...
  return (
    <DndProvider backend={HTML5Backend}>
      ...
      />
      <AudioInsertForm open={opendialogInsertAudio} handleAddMusic={handleAddMusic}
        handleClose={handleClose2} videoList={filtered} audioList={audioGallery} />
